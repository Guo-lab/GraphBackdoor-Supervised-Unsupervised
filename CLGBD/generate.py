...
def main(args_test):
    ...
    if args_test.dataset in GRAPH_CLASSIFICATION_DSETS:
        # 下流任务 trigger 注入
        train_dataset = injected_GraphClassificationDataset( # GraphDataset -> NodeClassificationDataset -> GraphClassificationDataset -> GraphClassificationDatasetLabeled
        ...
        clean_train_dataset = GraphClassificationDataset( # GraphDataset -> NodeClassificationDataset -> GraphClassificationDataset -> GraphClassificationDatasetLabeled
            dataset=args_test.dataset,
            rw_hops=args.rw_hops,
            subgraph_size=args.subgraph_size,
            restart_prob=args.restart_prob,
    ...
    # clean_emb = test_moco(clean_train_loader, clean_model, args)
    
    np.save(os.path.join(args.model_folder, args_test.dataset), emb.numpy())
    
    # np.save(os.path.join(args.model_folder, args_test.dataset), clean_emb.numpy())
    # np.save(os.path.join(args.model_folder, args_test.dataset + '_clean'), clean_emb.numpy())
    ##############
